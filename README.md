...
<p align="center">
  <img src="docs/img/pipeline_flow.svg" alt="The journey of a document, left to right. ① CRACK — a sample document is shown with its sensitive parts highlighted in colour (a header logo, a headshot, a contact line inside the body narrative, and an embedded ID/screen), and colour-matched arrows route each part to the model that handles it. ② FIND — logos (Grounding DINO), faces (YuNet), text/PII (ai_parse_document elements), and optional sensitive items (Claude vision). ③ DECIDE — a flat-fill + CLIP gate masks real brand marks and keeps icons and your own logo; faces are padded and Gaussian-blurred; text is split into per-line boxes so regex + a Claude classifier mask only the sensitive data fields and keep the narrative; the vision lane returns only sensitive regions, falling back to Grounding DINO. ④ MERGE — every box lands on one aligned coordinate frame (clip · pad · NMS). ⑤ OUTPUT — a non-destructive labelled overlay and the same document redacted (logo blacked, face blurred, contact line and ID blacked, narrative kept). Only the text lane literally cracks the page into elements; the logo, face and item models each scan the whole page in place — colour shows which model owns which part." width="960">
</p>

<sub>Follow one document left to right: it's **① CRACKED** into parts and each part is routed to the model that handles it (only the text lane literally cracks the page into elements via `ai_parse_document`; the logo, face and item models each scan the whole page in place — colour shows which model owns which part). Each lane is then the same two-step funnel: **② FIND** where content might be (Grounding DINO / YuNet / `ai_parse_document` / Claude vision), then **③ DECIDE** what actually gets masked (the flat-fill + CLIP gate for logos, always-blur for faces, the regex + Claude classifier for text). **④ MERGE** lands every box on one aligned coordinate frame; **⑤ OUTPUT** is a non-destructive labelled **overlay** and the **redacted copy** — the same document with its parts masked and the narrative kept. Lanes run in parallel and toggle independently — logos + faces are on by default, text/PII needs Databricks, and the **sensitive-items lane (dashed = optional, off by default)** adds a Claude-vision pass for in-image PII the document-text lane can't read. See [How logo detection works](#how-logo-detection-works-grounding-dino--crop--clip) for the crop → CLIP detail and [Known limitations](#known-limitations) for the document-vs-photo scope.</sub>
